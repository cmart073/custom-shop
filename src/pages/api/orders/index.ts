...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #1e3a2f 0%, #2d5a47 100%); padding: 30px; border-radius: 12px 12px 0 0;">
    <h1 style="color: #fff; margin: 0; font-size: 24px;">CMart073</h1>
  ...
  <div style="background: #fff; padding: 30px; border: 1px solid #e5e5e5; border-top: none;">
    <h2 style="margin: 0 0 20px 0; color: #1e3a2f;">Thank you, ${fullName}!</h2>
    <p>We've received your customization request. Your order reference is:</p>
    ...
    <h3 style="color: #1e3a2f; border-bottom: 1px solid #e5e5e5; padding-bottom: 10px;">Next Steps</h3>
    <div style="background: #f8faf9; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
      ...
      <p style="margin: 0; padding: 15px; background: #fff; border-radius: 6px; border-left: 4px solid #1e3a2f;">
        ${(env.SHIP_TO_ADDRESS || 'CMart073').replace(/,/g, '<br>')}
      </p>
    </div>
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
      ...
      <tr><td style="padding: 12px 0;"><strong>Turnaround:</strong></td><td style="text-align: right; padding: 12px 0;">2–5 business days after receipt</td></tr>
    </table>
    <div style="background: #fffbeb; border: 1px solid #f59e0b; border-radius: 8px; padding: 15px;">
      <p style="margin: 0; font-size: 14px; color: #92400e;"><strong>Please note:</strong> All services are cosmetic only.</p>
    </div>
  </div>
</body>
</html>`.trim();
  ...
  try {
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        from: { email: env.FROM_EMAIL || 'noreply@cmart073.com', name: 'CMart073' },
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (env.SENDGRID_API_KEY) {
      sendCustomerEmail(shortId, input.fullName, input.email, input.serviceType, estPriceMin, estPriceMax, env);
      if (env.ADMIN_EMAIL) {
        sendAdminEmail(shortId, input.fullName, input.email, input.serviceType, estPriceMin, estPriceMax, orderId, env);
      }
    }
